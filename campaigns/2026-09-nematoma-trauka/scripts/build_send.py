...
REMOTE_DIRECTORY = "campaigns/2026-09-nematoma-trauka"


def compact(fragment: str) -> str:
    fragment = re.sub(
        r"<style>(.*?)</style>",
        lambda match: "<style>" + re.sub(r"\s+", " ", match.group(1)).strip() + "</style>",
        fragment,
        flags=re.DOTALL,
    )
...
def main() -> None:
    ...
    send = compact(source.replace('src="assets/', f'src="{cdn_root}assets/'))
    for filename in (
