...
async function main() {
  ...
  for (const size of [
    ...
    await page.goto(pathToFileURL(path.join(campaign, 'newsletter.html')).href, { waitUntil: 'load' });
    await page.waitForFunction(
      () => Array.from(document.images).every((image) => image.complete && image.naturalWidth > 0),
      null,
      { timeout: 15000 },
    );
    await page.screenshot({ path: path.join(campaign, size.file), fullPage: true });
    await page.close();
  }
